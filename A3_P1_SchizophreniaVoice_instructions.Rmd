...
#We want to make a function that can take every txt.file in the Pitch folder and separate them and get a column for study, diagnosis, subject and trial

#We now begin to make the function
read_pitch <- function(filename) {

#We tell the function to paste each pitch-file according to how the filename is read
filename <- paste("data/Pitch/", filename, sep="")

#We then put the filename into a new variable we call pitch
pitch <- read_tsv(filename)                                                     

#Now we need to parse the filename to extract study, diagnosis, subject and trial. Parse means to extract the relevant things from the filename. Parsing is done by str_extract() or str_split() or str_match(). These functions use regular expressions which can be found on www.regex1001.com)

Study <- filename %>% str_extract("Study\\d")

Diagnosis <- filename %>% str_extract("D\\d")

Subject <- filename %>% str_extract("S\\d+")

Trial <- filename %>% str_extract("T\\d+")
    
#We now extract the pitch descriptors (mean, sd, iqr, etc). f0 is the column with all of the pitch values
Mean <- mean(pitch$f0)
...
#We then combine all this data in one dataset
return(data_frame(Study, Subject, Diagnosis, Trial, Mean, IQR, SD))

}

#We now test it on just one file while writing the function
test_data = read_pitch("Study1D0S101T7_f0.txt")

#Now we have created a function that works for one pitch-file, and we can now apply this function to all of the files in the Pitch folder
pitch_data = list.files(path = "data/Pitch/", pattern = ".txt") %>% 
    purrr::map_df(read_pitch)

#Now we write the data into a csv.file
write.csv(pitch_data, "data/pitch_data.csv")
...
demographic_clinical_data <- read.csv("data/DemographicData.csv", sep = ";")

#We filter away everything else than the Danish data (study 1-4)
demographic_clinical_data <- filter(demographic_clinical_data, Study == 1:4)

#Then duration data
duration_data <- read.delim("Data/Articulation.txt", sep = ",")

#We only want the Danish data (study 1-4)
duration_data$Study <- str_split(duration_data$soundname, "D", simplify = TRUE)

#We then need to remove all characters except for the number
duration_data$Study <- gsub("[a-zA-Z ]", "", duration_data$Study)


#Finally the pitch data
pitchData <- read.csv("data/pitch_data.csv")

pitchData$Study <- gsub("[a-zA-Z ]", "", pitchData$Study)

#Now we merge them
merged_data <- merge(demographic_clinical_data, duration_data)

merged_data <- merge(merged_data, pitchData)

write.csv(merged_data, "data/merged_data")

merged_data <- read.csv("data/merged_data", sep = ";")
# Now we save them
